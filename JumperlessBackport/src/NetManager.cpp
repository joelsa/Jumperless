...
void listNets(int output) // list nets doesnt care about debugNM, don't call it if you don't want it to print
{
    if (output != 1) //1 only prints to the arduino
    {
...
if (output >= 1)
{
    ...
    else
    {
        ...
        for (int i = 8; i < MAX_NETS; i++)
        {
            if (net[i].number == 0 || net[i].nodes[0] == -1) // stops searching if it gets to an unallocated net
            {
                //Serial1.print("Done listing nets");
                break;
            }
            ...
            Serial1.print(i);
            Serial1.print("\t");
            int netNameLength = Serial1.print(net[i].name);
            if (netNameLength < 8)
            {
                Serial1.print("\t");
            }
            Serial1.print("\t");
            Serial1.print(net[i].number);
            Serial1.print("\t0x");
        netNameLength = Serial1.print(net[i].rawColor, HEX);
        if (netNameLength < 6)
        {
            Serial1.print("\t");
        }
...
void listSpecialNets(int output)
{
    if (output != 1)
    {
...
if (output >= 1)
{
    ...
    for (int i = 0; i < 8; i++)
    {
        ...
        {
            tabs += printNodeOrName(net[i].nodes[j]);
            // tabs += Serial1.print(definesToChar(net[i].nodes[j]));
...
int printNodeOrName(int node, int longOrShort, int serial) // returns number of characters printed (for tabs)
{
    // Serial.print("serial = ");
    ...
    if (node >= 100)
    {
        if (serial == 1)
        {
            strcpy(serial1message, definesToChar(node, longOrShort));
            serial1messageIndex = strlen(serial1message);
            serial1messageReady = 1;
            
            return serial1messageIndex;
        }
        else
        {
